...
dataSource, preProcessing, pca, modelling, implementation = st.tabs(["Data Source", "Preprocessing", "PCA","Modelling", "Implementation"])
...
with pca:
     ...
     st.write("Menampilkan data setelah PCA")
     df_pca = pd.DataFrame(X_pca, columns=['PC1', 'PC2'])
     df_pca['Outcome'] = y
     st.table(df_pca.head(10))

     # Transformasi data uji menggunakan objek scaler
     xtest_pca = scaler.transform(xtest_pca)
...
with modelling:
    ...
    # KNN dengan PCA
     ...
     accuracy_knn_pca = accuracy_score(ytest, y_pred_knn_pca)
     st.success("Hasil akurasi KNN dengan PCA: " + str(round(accuracy_knn_pca,4)*100) + "%")

    # Gaussian Naive Bayes dengan PCA
     st.header("Gaussian Naive Bayes dengan PCA")
     clf_pca = GaussianNB()
     clf_pca.fit(xtrain_pca, ytrain)
     y_pred_clf_pca = clf_pca.predict(xtest_pca)
     accuracy_clf_pca = accuracy_score(ytest, y_pred_clf_pca)
     st.success("Hasil akurasi Gaussian Naive Bayes dengan PCA:" + str(round(accuracy_clf_pca,4)*100) + "%")

    # Decision Tree dengan PCA
     st.header("Decision Tree dengan PCA")
     d3_pca = DecisionTreeClassifier()
     d3_pca.fit(xtrain_pca, ytrain)
     y_pred_d3_pca = d3_pca.predict(xtest_pca)
     accuracy_d3_pca = accuracy_score(ytest, y_pred_d3_pca)
     st.success("Hasil akurasi Decision Tree dengan PCA:" + str(round(accuracy_d3_pca,4)*100) + "%")

    # Artificial Neural Network dengan Backpropagation dengan PCA
     st.header("Artificial Neural Network dengan Backpropagation dengan PCA")
     model_pca = Sequential()
     model_pca.add(Dense(16, input_dim=X_pca.shape[1], activation='relu'))
     model_pca.add(Dense(12, activation='relu'))
     model_pca.add(Dense(1, activation='sigmoid'))
     model_pca.compile(loss='binary_crossentropy', optimizer='adam', metrics=['accuracy'])
     model_pca.fit(xtrain_pca, ytrain, epochs=10, batch_size=10, verbose=0)
     _, accuracy_annbp_pca = model_pca.evaluate(xtest_pca, ytest)
     st.success("Hasil Artificial Neural Network dengan Backpropagation dengan PCA:" + str(round(accuracy_annbp_pca,4)*100) + "%")
